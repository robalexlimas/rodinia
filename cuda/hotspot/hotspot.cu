...
__global__ void calculate_temp(int iteration, // number of iteration
    ...
    __shared__ float
        temp_t[BLOCK_SIZE][BLOCK_SIZE]; // saving temparary temperature result
    ...
    if (IN_RANGE(loadYidx, 0, grid_rows - 1) &&
        ...
        temp_on_cuda[ty][tx] = temp_src[index]; // Load the temperature data
                                                // from global memory to shared
                                                // memory
