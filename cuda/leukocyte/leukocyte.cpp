...
int run(int argc, char **argv) {
    ...
    if (argc != 3) {
        fprintf(stderr, "usage: %s <input file> <number of frames to process>",
                argv[0]);
        exit(1);
    }
    ...
    int i, j, *crow, *ccol, pair_counter = 0, x_result_len = 0, Iter = 20,
                            ns = 4, k_count = 0, n;
    MAT *cellx, *celly, *A;
        ...
        if ((G[n] < -1 * threshold) || G[n] > threshold) {
            ...
            if ((m_min(x) > b) && (m_min(y) > b) &&
                ...
                for (i = 0; i < Iter; i++) {
                    ...
                    splineenergyform01(Cx, Cy, grad_x, grad_y, ns, delta,
                                       2.0 * dt, typeofcell);
                }
                ...
                // Find the cell centers by computing the means of X and Y
                // values for all snaxels of the spline contour
